...
const scoreBook = (
  ...
  const targetProgress = getTargetBookProgressionIndex({ grade, level });
...
const selectBooks = (
  ...
  return [...availableBooks]
    .sort((left, right) => {
      const scoreGap = scoreBook(right, grade, level, keywords, intensity === LearningPreferenceIntensity.INTENSIVE)
        - scoreBook(left, grade, level, keywords, intensity === LearningPreferenceIntensity.INTENSIVE);
