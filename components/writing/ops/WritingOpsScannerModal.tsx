      ...
      <div>
        ...
        <div className="mt-6 space-y-4">
          <div>
            ...
            {scannerFiles.length > 0 ? (
              <div className="mt-3 grid gap-2">
                {fileRows.map(({ file, resolvedMimeType, kind }) => (
                  <div key={`${file.name}-${file.size}`} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="font-bold text-slate-950">{file.name}</div>
                        <div className="mt-1 text-xs text-slate-500">{resolvedMimeType || '形式未判定'} / {formatWritingUploadBytes(file.size)}</div>
                      </div>
            ...
            <div className={`mt-3 flex items-start gap-2 rounded-2xl border px-4 py-3 text-sm font-bold ${validationTone}`}>
