  ...
  const createDisabledReason = !trimmedTitle
    ...
    >
      <div data-testid="phrasebook-create-modal" className="safe-pad-top sticky top-0 z-10 border-b border-slate-100 bg-white/96 px-4 pb-4 pt-4 backdrop-blur sm:rounded-t-[32px] sm:px-6">
        <button onClick={onClose} aria-label="閉じる" className="absolute right-4 top-4 rounded-lg p-2 font-bold text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600">
          <X className="h-5 w-5" />
        </button>
            ...
            />
	          ...
	          <div>
	            ...
	            <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center transition-colors hover:border-medace-500">
	              <input type="file" id={fileInputId} accept=".pdf,image/*" className="hidden" onChange={onFileChange} />
      ...
      <MobileStickyActionBar className="safe-pad-bottom border-t border-slate-100 bg-white/96 px-4 py-4 backdrop-blur sm:px-6 sm:rounded-b-[32px]">
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <button
            ...
            className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-medace-600 px-5 py-3 font-bold text-slate-950 transition-colors hover:bg-medace-700 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
