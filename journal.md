...
- `components/dashboard/BusinessAdminDashboardSections.tsx` と `services/storage.ts` の分割計画。

## Next Focus
- `todo.md` の High priority に合わせ、割当運用の履歴化、cohort 単位運用、B2B KPI の継続検証を先に固める。
- `components/dashboard/BusinessAdminDashboardSections.tsx` と `services/storage.ts` の分割を優先し、B2B 導線の変更コストを下げる。
