...
const isDuplicateSubmissionAttemptError = (error: unknown): boolean => {
  ...
  return (
    ...
    && message.includes('writing_submissions.assignment_id')
    && message.includes('writing_submissions.attempt_no')
  );
};

export const commitFinalizedSubmission = async (
  env: AppEnv,
  params: {
    submissionId: string;
    assignmentId: string;
  ...
  try {
    ...
    if (isDuplicateSubmissionAttemptError(error)) {
...
export const commitTeacherReviewDecision = async (
  env: AppEnv,
  params: {
    submissionId: string;
    reviewId: string;
